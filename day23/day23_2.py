...
class Network(object):
    ...
    def check_network(self):
        ...
        #     print(self.packet_buffer[Network.NAT_ADDRESS])
        for computer in self.computers:
            if not computer.idle:
                # print(computer)
                break
        else:
            if not self.packet_buffer[Network.NAT_ADDRESS]:
                return 
            nat_value = self.packet_buffer[Network.NAT_ADDRESS].pop()
            if self.last_nat_packet == nat_value:
                print(nat_value)
                self.done = True
            self.last_nat_packet = nat_value
            self.packet_buffer[0].append(nat_value)
